...
3. Pick the runners. Use `arena runners` from `~/.cursor/rules/pstack-models.mdc` when present. Otherwise default to one each on `claude-fable-5-thinking-max`, `gpt-5.6-sol-max`, `grok-4.6-fast-xhigh`, `claude-opus-5-thinking-xhigh`. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
   **Model floor: Opus.** Never run a candidate on a model weaker than Opus-tier (no Sonnet, no Haiku) — a weak candidate wastes a slot and pollutes the field with a design the graft phase must argue down. When the environment's model roster doesn't include the named defaults, map each slot to the strongest available tier and fill remaining slots by repeating Opus-or-stronger models rather than reaching down a tier.
4. Assign output paths. Each candidate writes to its own location (`/tmp/arena-<slug>/candidate-<n>/`). N candidates writing to the same path is shared mutable state and fails the the **separate-before-serializing-shared-state** principle skill test.
